...
public class WeightedGraph {
    ...
    private boolean hasCycle(Node node, Node parent, Set<Node> visisted) {
        visisted.add(node);
        for(var edge : node.getEdges()) {
            if(edge.to == parent) {
                continue;
            }
            if(visisted.contains(edge.to) || hasCycle(edge.to, node, visisted)){
                return true;
            }
        }
        return false;
